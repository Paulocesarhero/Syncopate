...
export const DifficultySelector = memo(function DifficultySelector({ difficulty, onChange }: DifficultySelectorProps) {
  return (
        ...
        {options.map((option) => (
          <button
            ...
            className={`difficulty-selector__option ${
              difficulty === option.value ? 'difficulty-selector__option--active' : ''
            }`}
            onClick={() => onChange(option.value)}
          >
            <span className="difficulty-selector__option-label">{option.label}</span>
            <span className="difficulty-selector__option-desc">{option.description}</span>
          </button>
        ))}
      </div>
    </div>
